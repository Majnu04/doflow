    ...
    try {
      ...
      const [sectionsRes, problemsRes, courseRes, enrollmentsRes] = await Promise.all([
        ...
        retryWithBackoff(() => api.get('/payment/enrollments'), 2, 500)
          .catch(() => ({ data: [] })) // Fallback to empty array if not authenticated
      ]);
      ...
      console.error('fetchDsaCourseData error:', error);
      return rejectWithValue(error.response?.data?.message || error.message || 'Failed to fetch DSA course data');
    }
  }
);
