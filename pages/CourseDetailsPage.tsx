...
const LockedCurriculum: React.FC<{ modules: Course['modules']; isDsa: boolean; hasVideoLessons: boolean }> = ({ modules, isDsa, hasVideoLessons }) => (
        ...
        {modules.map((module, index) => (
            ...
            >
                ...
                <div className="border-t border-border-subtle bg-gradient-to-b from-white/50 to-light-bg p-8 text-center">
                    ...
                    <p className="text-sm text-light-textMuted mt-1">Purchase this course to unlock all {isDsa ? 'coding problems' : (hasVideoLessons ? 'video lessons' : 'lessons')}</p>
                </div>
            </div>
        ))}
    </div>
                            ...
                            <li key={lesson.id || `lesson-${index}-${lessonIndex}`} className="flex justify-between items-center text-light-text">
                                <div className="flex items-center">
                                                                        {lesson.videoUrl ? (
                                                                            ...
                                                                            <svg className="w-5 h-5 mr-3 text-brand-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 20h9"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4h9"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 9h16"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 15h16"></path></svg>
                                                                        )}
...
const CourseDetailsPage: React.FC<{ courseId: string }> = ({ courseId }) => {
    ...
    const hasVideoLessons = useMemo(() => {
        const sections = metadata.sections || [];
        for (const section of sections) {
            for (const lesson of (section?.lessons || [])) {
                if (lesson?.videoUrl && String(lesson.videoUrl).trim().length > 0) return true;
            }
        }
        return false;
    }, [metadata.sections]);
                                                ...
                                                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-amber-700 mt-1">
                                                            {isDsa ? 'Problem names and details are hidden. ' : (hasVideoLessons ? 'Video lessons and resources are hidden. ' : 'Lessons and resources are hidden. ')}
                                                            Purchase this course to unlock the full curriculum.
                                                        </p>
                                                    </div>
                                                </div>
                                                <LockedCurriculum modules={metadata.sections} isDsa={isDsa} hasVideoLessons={hasVideoLessons} />
                                            </>
                                        )
                                    )}
                                </div>
