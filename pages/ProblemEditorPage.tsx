...
import { FiPlay, FiCheck, FiBookmark, FiStar, FiChevronLeft, FiLoader, FiExternalLink, FiRefreshCw, FiRotateCcw } from 'react-icons/fi';
...
import { explainSolutionComplexity, askAiDoubt } from '../services/geminiService';
...
const LANGUAGE_SIGNATURES: Record<string, string> = {
  ...
  python: 'def <name>(...):',
  cpp: '<return_type> <name>(...)',
  java: 'class Solution { static <type> <name>(...) }',
  c: '<return_type> <name>(...)',
};

const stripInlineComments = (line: string) => {
  // Keep it simple: remove trailing //... and #...
  return line.replace(/\s*\/\/.*$/, '').replace(/\s*#.*$/, '').trim();
};

const stripTrailingBlockStart = (line: string) => {
  return line.replace(/\{\s*$/, '').trim();
};

const findSignatureNearMarker = (snippet: string) => {
  const lines = snippet.split(/\r?\n/);
  const markerIndex = lines.findIndex((l) => /Write your (logic|code) here/i.test(l));
  ...
  for (let i = markerIndex; i >= 0; i -= 1) {
    ...
    if (!trimmed.includes(')') && !trimmed.startsWith('def ')) continue;
    return stripTrailingBlockStart(stripInlineComments(trimmed));
  }
  return null;
};
...
const deriveSignature = (language: string, snippet: string) => {
  ...
  const markerSignature = findSignatureNearMarker(snippet);
  if (markerSignature) return markerSignature;
  ...
  if (language === 'java') {
    const methodMatch = snippet.match(/\bpublic\s+static\s+[\w<>\[\]]+\s+[A-Za-z_]\w*\s*\([^)]*\)/m)
      || snippet.match(/\bstatic\s+[\w<>\[\]]+\s+[A-Za-z_]\w*\s*\([^)]*\)/m);
    if (methodMatch?.[0]) {
      return stripTrailingBlockStart(stripInlineComments(methodMatch[0]));
    }
  }
  ...
  if (language === 'python') {
    ...
    if (methodLine) {
      return stripInlineComments(methodLine.replace(/:\s*$/, ''));
    }
  }

  if (language === 'javascript') {
    const signatureLine = lines.find((line) => /^function\s+\w+\s*\(/.test(line) || /^const\s+\w+\s*=\s*\(?.*\)\s*=>/.test(line));
    if (signatureLine) return stripTrailingBlockStart(stripInlineComments(signatureLine));
  }

  if (language === 'cpp' || language === 'c') {
    const signatureLine = lines
      .filter((line) => !line.startsWith('#') && !line.startsWith('typedef') && !line.startsWith('using ') && !line.startsWith('struct ') && !line.startsWith('class '))
      .find((line) => line.includes('(') && (line.includes(')') || line.endsWith(')')));
    if (signatureLine) return stripTrailingBlockStart(stripInlineComments(signatureLine));
  }

  return stripTrailingBlockStart(stripInlineComments(lines[0]));
};

const getExpectedEntryName = (language: string, snippet: string) => {
  const src = String(snippet || '');
  if (!src.trim()) return null;
  if (language === 'javascript') {
    const m = src.match(/\bfunction\s+([A-Za-z_]\w*)\s*\(/);
    return m?.[1] || null;
  }
  if (language === 'python') {
    const m = src.match(/\bdef\s+([A-Za-z_]\w*)\s*\(/);
    return m?.[1] || null;
  }
  if (language === 'java') {
    const m = src.match(/\bpublic\s+static\s+[\w<>\[\]]+\s+([A-Za-z_]\w*)\s*\(/)
      || src.match(/\bstatic\s+[\w<>\[\]]+\s+([A-Za-z_]\w*)\s*\(/);
    return m?.[1] || null;
  }
  if (language === 'cpp' || language === 'c') {
    const cleanedLines = src.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
    const candidate = cleanedLines
      .filter((line) => !line.startsWith('#') && !line.startsWith('typedef') && !line.startsWith('using ') && !line.startsWith('struct ') && !line.startsWith('class '))
      .find((line) => line.includes('('));
    if (!candidate) return null;
    const matches = candidate.match(/\b([A-Za-z_]\w*)\s*\(/g);
...
const ProblemEditorPage: React.FC<ProblemEditorPageProps> = ({ problemId }) => {
  ...
  const [activeTab, setActiveTab] = useState<'description' | 'submissions' | 'notes' | 'ai'>('description');
  ...
  const [aiStatus, setAiStatus] = useState<'idle' | 'loading' | 'failed'>('idle');
  ...
  const getStarterSnippetFor = (lang: typeof SUPPORTED_LANGUAGES[number]) => {
    const entry = problem?.starterCode?.find((sc: any) => sc.language === lang) || null;
    const rawSnippet = entry?.visibleCode ?? entry?.code ?? DEFAULT_SNIPPETS[lang] ?? '';
    return entry?.visibleCode ? rawSnippet : (extractSolveSection(lang, rawSnippet) || rawSnippet);
  };

  const handleResetCode = () => {
    if (!problem) return;
    setCode(getStarterSnippetFor(language));
    dispatch(clearRunResults());
    dispatch(clearSubmitResults());
    setShowResults(false);
  ...
  const handleRefreshStarter = async () => {
    if (!problemId) return;
    try {
      dispatch(clearRunResults());
      dispatch(clearSubmitResults());
      setShowResults(false);
      await (dispatch(fetchProblemData(problemId)) as any).unwrap?.();
      toast.success('Starter refreshed from server.');
    } catch (err: any) {
      toast.error(err?.message || 'Failed to refresh starter.');
    }
  };

  const preflightValidate = () => {
    const starterSnippet = starterEntry?.visibleCode ?? starterEntry?.code ?? '';
    const expectedName = getExpectedEntryName(language, starterSnippet);
    if (!expectedName) return true;

    const userSrc = String(code || '');
    const hasName = new RegExp(`\\b${expectedName}\\b`).test(userSrc);
    if (!hasName) {
      toast.error(`Don't rename/remove the required entry method. Expected: ${expectedName}(...)`);
      return false;
    }
    return true;
  };
  ...
  const handleAskAi = async () => {
    ...
    if (!q) {
      toast.error('Type your doubt first.');
      return;
    }
    ...
    try {
      ...
      const answer = await askAiDoubt({
        question: q,
        problemTitle: problem?.title,
        problemDescription: (problem as any)?.description,
        examples: (problem as any)?.examples,
        language,
        code,
      });

      setAiMessages((prev) => [...prev, { role: 'assistant', text: answer || 'No answer returned.' }]);
      setAiStatus('idle');
    } catch (error: any) {
      console.error('AI doubt failed:', error);
      setAiStatus('failed');
      toast.error(error?.message || 'AI is unavailable right now.');
    }
  };
  ...
  const hasFetchedSubmissionsForProblem = useRef<string | null>(null);
  useEffect(() => {
    if (activeTab !== 'submissions') return;
    if (!isAuthenticated) return;
    if (!problemId) return;

    // Fetch once per problem on-demand (covers the case where the page loaded before login).
    if (hasFetchedSubmissionsForProblem.current !== problemId) {
      hasFetchedSubmissionsForProblem.current = problemId;
      dispatch(fetchProblemSubmissions(problemId));
    }
  }, [activeTab, isAuthenticated, problemId, dispatch]);

  useEffect(() => {
    if (!isAuthenticated) {
      hasFetchedSubmissionsForProblem.current = null;
    }
  }, [isAuthenticated]);
  useEffect(() => {
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        ...
        <div className="w-full lg:w-1/2 border-b lg:border-b-0 lg:border-r border-light-border dark:border-dark-border flex flex-col min-h-0">
          <div className="flex border-b border-light-border dark:border-dark-border bg-light-card dark:bg-dark-card">
            {(['description', 'submissions', 'notes', 'ai'] as const).map(tab => (
              <button
                key={tab}
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-light-bg dark:bg-dark-bg">
            ...
            {activeTab === 'submissions' && (
              !isAuthenticated ? (
                ...
                <div className="flex items-center justify-center py-16">
                  <div className="flex items-center gap-3 text-light-textMuted dark:text-dark-muted">
                ...
                <ErrorState
                  message={submissionsError || 'Failed to load submissions.'}
                  onRetry={() => dispatch(fetchProblemSubmissions(problemId))}
                />
              ) : submissionList.length === 0 ? (
                <EmptyState title="No Submissions Yet" message="Your previous submissions (if any) will appear here." />
              ) : (
            ...
            {activeTab === 'ai' && (
              <div className="space-y-4">
                <section>
                  <h3 className="text-lg font-semibold text-light-text dark:text-dark-text mb-2">Ask AI</h3>
                  <p className="text-sm text-light-textSecondary dark:text-dark-muted">
                    Ask doubts about approach, edge cases, or why your logic fails.
                  </p>
                </section>

                <div className="border border-light-border dark:border-dark-border rounded-2xl bg-white/80 dark:bg-dark-card p-4">
                  <div className="space-y-3 max-h-80 overflow-y-auto">
                    {aiMessages.length === 0 ? (
                      <p className="text-sm text-light-textSecondary dark:text-dark-muted">No messages yet.</p>
                    ) : (
                      aiMessages.map((m, idx) => (
                        <div
                          ...
                          className={`rounded-xl p-3 text-sm whitespace-pre-wrap ${
                            m.role === 'user'
                              ? 'bg-light-cardAlt dark:bg-dark-cardAlt text-light-text dark:text-dark-text border border-light-border dark:border-dark-border'
                              : 'bg-white dark:bg-dark-card text-light-textSecondary dark:text-dark-muted border border-light-border dark:border-dark-border'
                          }`}
                        >
                          <p className="text-[10px] uppercase tracking-widest mb-1 text-light-textMuted dark:text-dark-muted">
                            {m.role === 'user' ? 'You' : 'AI'}
                          </p>
                          {m.text}
                        </div>
                  ...
                  <div className="mt-4 grid gap-3">
                    <textarea
                      ...
                      className="w-full h-28 p-4 rounded-2xl border border-light-border dark:border-dark-border bg-white/80 dark:bg-dark-card text-sm text-light-text dark:text-dark-text focus:outline-none focus:border-brand-primary resize-none"
                      placeholder="Type your doubt..."
                      maxLength={2000}
                      disabled={aiStatus === 'loading'}
                    />
                    <div className="flex items-center justify-between text-xs text-light-textMuted dark:text-dark-muted">
                      <span>{aiQuestion.length}/2000 characters</span>
                      {aiStatus === 'failed' && <span className="text-red-500">AI unavailable</span>}
                    </div>
                    <div className="flex items-center gap-3">
                      <Button variant="primary" size="sm" onClick={handleAskAi} disabled={aiStatus === 'loading' || !problem}>
                        {aiStatus === 'loading' ? 'Thinking...' : 'Ask AI'}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
        ...
        <div className="w-full lg:w-1/2 flex flex-col min-h-0">
          <div className="flex items-center justify-between px-4 py-2 border-b border-light-border dark:border-dark-border bg-light-card dark:bg-dark-card">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={handleRefreshStarter} disabled={isRunning || status === 'loading'}>
