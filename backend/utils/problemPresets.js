...
function lengthOfLongestSubstring(s) {
  ...
  return 0;
        ...
        adapterCode: `function __doflow_entry(...args) {
  const [s] = args;
  const inputStr = typeof s === 'string' ? s : String(s ?? "");
  const result = lengthOfLongestSubstring(inputStr);
...
`,
    ...
    solver = Solution()
    result = solver.lengthOfLongestSubstring(str(s))
    return str(result)
`,
        code: `class Solution:
    def lengthOfLongestSubstring(self, s: str) -> int:
...
`,
    ...
    public int lengthOfLongestSubstring(String s) {
        ...
        adapterCode: `final class DoFlowAdapter {
    private DoFlowAdapter() {}
    ...
    public int lengthOfLongestSubstring(String s) {
      ...
      {
        language: 'cpp',
        visibleCode: `#include <bits/stdc++.h>
using namespace std;
...
public:
    int lengthOfLongestSubstring(string s) {
        ...
        adapterCode: `#include <bits/stdc++.h>
using namespace std;

string __doflow_entry(const vector<string>& args) {
    string s = args.size() > 0 ? args[0] : "";
...
int lengthOfLongestSubstring(char * s) {
  ...
  return 0;
    ...
    hints: [
      'Generate all substrings and check for repetitions (Brute Force).',
      'Use a sliding window approach with two pointers (left, right) to optimize.',
      'Use a Hash Set or Hash Map to store characters in the current window to allow O(1) lookups.',
    ],
    examples: [
      {
        input: 'abcabcbb',
