...
import { buildHarnessedSource, buildBatchHarnessedSource } from '../utils/compilerHarness.js';
...
const executeTestsSequentially = async (code, language, testCases = [], options = {}) => {
  const { stopOnFailure = false, problem, batchHidden = false } = options;
  ...
  if (hiddenTests.length > 0) {
    const canBatch = Boolean(batchHidden) && Boolean(stopOnFailure) && hiddenTests.length > 1;
    if (canBatch) {
      console.log(`🚀 Executing ${hiddenTests.length} hidden test(s) in a single batch submission...`);

      const harnessedSource = buildBatchHarnessedSource(language, code, hiddenTests, adapterCode, entryInvocation);
      const payload = {
        language_id: languageId,
        source_code: harnessedSource,
        expected_output: 'OK',
      ...
      const stdout = (rawResult.stdout || '').trim();
      const isAccepted = rawResult.status?.id === 3;
      const batchPassed = isAccepted && stdout === 'OK';
      ...
      if (batchPassed) {
        for (let i = 0; i < hiddenTests.length; i += 1) {
          ...
          results.push({
            testCase: originalIndex + 1,
            originalIndex,
            input: testCase.input,
            expectedOutput: typeof testCase.expectedOutput === 'string' ? (testCase.expectedOutput || '').trim() : undefined,
            actualOutput: typeof testCase.expectedOutput === 'string' ? (testCase.expectedOutput || '').trim() : 'OK',
            errorOutput: '',
            executionTime: null,
            memory: null,
            explanation: testCase.explanation,
        ...
        console.log('⚠️ Hidden batch failed; re-running hidden tests sequentially to find the first failing case...');
        for (let i = 0; i < hiddenTests.length; i += 1) {
          const testCase = hiddenTests[i];
          const originalIndex = publicTests.length + i;

          const perCaseSource = buildHarnessedSource(language, code, testCase, adapterCode, entryInvocation);
          const perCasePayload = {
            language_id: languageId,
            source_code: perCaseSource,
          };

          const expectedOutput = coerceExpectedOutput(testCase.expectedOutput);
          if (expectedOutput !== null) {
            perCasePayload.expected_output = expectedOutput;
          }

          const perCaseRaw = await submitSingleTestCase(client, perCasePayload);
          const formatted = formatJudge0Result(perCaseRaw, testCase, originalIndex);
          results.push(formatted);
          console.log(`  ↳ Hidden test ${i + 1}: ${formatted.passed ? '✅ PASSED' : '❌ FAILED'} (${formatted.status})`);

          if (formatted.passed === false) {
            break;
          }
        }
      }
...
export const submitCode = async (req, res) => {
  try {
    ...
    const evaluation = await executeTestsSequentially(code, language, orderedTestCases, { stopOnFailure: true, problem, batchHidden: true });
