...
export const resendOTP = async (req, res) => {
  try {
    ...
    // Keep pending name in sync with latest form input when provided.
    if (typeof name === 'string' && name.trim()) {
      pendingReg.name = name.trim();
    }
    ...
    if (emailResult.success) {
      ...
      const reason = isDev ? (emailResult.error || emailResult.message) : undefined;
      res.status(500).json({
        success: false,
...
export const sendOTP = async (req, res) => {
  try {
    ...
    // Send OTP email
    const emailResult = await sendRegistrationOTP(email, normalizedName, otp);
...
export const sendResetOTP = async (req, res) => {
  try {
    ...
    await PendingRegistration.deleteMany({ email, purpose: 'reset_password' });
...
export const verifyResetOTP = async (req, res) => {
  try {
    ...
    const pendingReset = await PendingRegistration.findOne({ email, purpose: 'reset_password' });
