  ...
  sections: [
    {
      ...
      lessons: [
          ...
          resources: [
            {
              ...
              url: JSON.stringify({
                type: "mcq",
                content: {
                  problemTitle: "Quiz: Output Fundamentals",
                  question: "What is the output of: print('5 + 3')",
                  options: ["A) 8", "B) 5 + 3", "C) 5+3", "D) Error"],
                  correctAnswer: "B",
                  ...
                  hint: "Remember the difference between quoted text (strings) and mathematical expressions"
                }
              }),
              type: "other"
            }
        ...
        },
        {
          ...
          resources: [
            {
              title: "Coding Challenge Content",
              url: JSON.stringify({
                type: "codingTask",
                content: {
                  problemTitle: "Create a Welcome Message",
                  problemStatement: "Write a program that prints a three-line welcome message to new Python learners.\n\nYour program should output exactly:\nHello, I am learning Python!\nPython is fun!\nLet's code together!",
                  constraints: "• Use exactly 3 print() statements\n• Output must match exactly as shown\n• Each line should be a separate print() call",
                  hint: "Remember: Strings are enclosed in single or double quotes. Each print() statement outputs one line.",
                  testCases: [
                    {
                      input: "",
...
async function seedPythonCourse() {
  try {
    ...
    // Generate the slug that will be auto-created by the Course model
    const autoGeneratedSlug = pythonCourse.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

    // Check if course already exists using the auto-generated slug
