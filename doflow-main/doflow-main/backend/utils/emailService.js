...
export const sendUpgradeReminderEmail = async (email, name, course) => {
  ...
  const mailOptions = {
    from: `"${process.env.EMAIL_FROM_NAME || 'DoFlow Academy'}" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: `Unlock ${courseTitle} - Premium Access Reminder`,
    html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
      ...
      <body style="margin:0;padding:0;background:#f5f7fa;font-family:Arial, sans-serif;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;">
          <tr>
            <td align="center">
              <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 6px 16px rgba(0,0,0,0.08);">
                <tr>
                  <td style="background:linear-gradient(135deg,#e06438 0%,#f3a45c 100%);padding:32px;text-align:center;color:#fff;">
                    <h1 style="margin:0;font-size:24px;">DoFlow Premium</h1>
                    <p style="margin:8px 0 0 0;opacity:0.9;">Unlock your full course access</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:32px;">
                    <p style="margin:0 0 16px 0;font-size:16px;color:#1a1f36;">Hi ${name},</p>
                    <p style="margin:0 0 24px 0;font-size:15px;color:#4e5d78;">You recently explored <strong>${courseTitle}</strong>. Upgrade now to unlock premium lessons, mock tests, and your certificate.</p>
                    <div style="background:#fef8f5;border:1px solid #f3a45c;border-radius:10px;padding:20px;margin-bottom:24px;">
                      <p style="margin:0 0 8px 0;font-size:14px;color:#6b7280;">Limited-time price</p>
                      <p style="margin:0;font-size:24px;color:#e06438;font-weight:bold;">₹${price}</p>
                    </div>
                    <div style="text-align:center;margin-bottom:24px;">
                      <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/#/checkout" style="display:inline-block;padding:12px 24px;background:#e06438;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;">Upgrade Now</a>
                    </div>
                    <p style="margin:0;font-size:13px;color:#8896ab;">If you already purchased, you can ignore this email.</p>
                  </td>
                </tr>
              </table>
            </td>
  ...
  try {
    ...
    return { success: false, message: 'Failed to send upgrade reminder email' };
  }
};
