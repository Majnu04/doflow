...
const CourseDetailsPage: React.FC<{ courseId: string }> = ({ courseId }) => {
    ...
    // Enrollment progress (mock data for now — will be connected to backend progress API)
    const enrollmentProgress = useMemo(() => {
        if (!isEnrolled) return null;
        const totalLessons = metadata.sections.reduce((sum: number, s: any) => sum + (s.lessons?.length || 0), 0);
        // In a real implementation, completedLessons would come from the progress API
        const completedLessons = 0;
        const progressPercent = totalLessons > 0 ? Math.round((completedLessons / totalLessons) * 100) : 0;
        return { completedLessons, totalLessons, progressPercent };
    }, [isEnrolled, metadata.sections]);
                                ...
                                <div className="bg-light-card border border-border-subtle rounded-3xl overflow-hidden shadow-xl">
                                    {metadata.promoVideo ? (
                                        ...
                                        <div className="relative">
                                            <img src={metadata.thumbnail} alt={course.title} loading="lazy" className="w-full object-cover aspect-video" />
                                        ...
                                        <div className="mt-5 space-y-3">
                                            ...
                                            {!enrollmentsLoading && isEnrolled ? (
                                                <>
                                                    {/* Enrollment Progress */}
                                                    {enrollmentProgress && (
                                                        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-1">
                                                            <div className="flex items-center justify-between mb-2">
                                                                <span className="text-xs font-semibold text-emerald-800">Your Progress</span>
                                                                <span className="text-xs font-bold text-emerald-700">{enrollmentProgress.progressPercent}%</span>
                                                            </div>
                                                            <ProgressBar
                                                                value={enrollmentProgress.progressPercent}
                                                                variant="success"
                                                                size="sm"
                                                            />
                                                            <p className="text-[10px] text-emerald-600 mt-1.5">
                                                                {enrollmentProgress.completedLessons} of {enrollmentProgress.totalLessons} {isDsa ? 'problems' : 'lessons'} completed
                                                            </p>
                                                        </div>
                                                    )}
