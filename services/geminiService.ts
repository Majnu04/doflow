...
export const explainSolutionComplexity = async ({ language, code, problemTitle, metrics }: ComplexityRequest): Promise<string> => {
  try {
    ...
    return String(response?.data?.insights ?? '').trim();
  } catch (error) {
    // Fallback to direct Gemini only if a client key is configured (useful in standalone frontend deployments).
    const client = getClient();
    if (!client) {
    ...
    const instructions = `You are an algorithms mentor. Given a ${language} solution and recent runtime/memory metrics, summarize the likely time and space complexity in Big-O terms. Provide:
...
${trimmedCode}`,
      ...
      console.error('Error generating complexity insights:', inner);
      throw new Error('Failed to generate complexity insights.');
    }
  }
};
...
export const askAiDoubt = async (payload: DoubtRequest): Promise<string> => {
